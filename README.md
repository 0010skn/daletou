...
系统新增了 AI 预测结果的访问控制功能：

- 用户必须输入有效的量子预测密钥才能查看 AI 预测结果
- 量子预测密钥价格为每次 500 元人民币
- 系统每天自动生成新的量子预测密钥，保存在日期命名的文件中（格式：`data/pin/YYYY-MM-DD.txt`）
- 页面设计了美观的输入界面，提升用户体验
- 完全在服务器端验证，确保预测结果的安全性
- 系统会为用户显示 MD5 加密后的身份验证信息

### 量子预测密钥管理

管理员可以通过以下方式查看和管理量子预测密钥：

1. 访问 `/api/key-info?admin_token=您的管理员令牌` 查看当前有效的密钥
2. 过期的密钥会自动备份到 `data/pin/expired/` 目录
3. 系统会在用户访问时自动检查并更新密钥，无需手动干预

### 使用流程

1. 用户访问 AI 预测结果区域
2. 系统显示锁定界面，并展示加密后的身份验证信息
3. 用户输入正确的量子预测密钥后，系统立即验证并解锁
4. 成功解锁后显示 AI 预测结果

## 管理员密钥配置

项目支持使用管理员密钥 API 来获取当前的量子预测密钥。

```
ADMIN_KEY=your_secure_admin_key_here
```

3. 将`your_secure_admin_key_here`替换为复杂且安全的密钥

管理员可以通过以下 API 获取当前的量子预测密钥：

```bash
curl -X POST -H "X-Admin-Key: your_secure_admin_key_here" https://your-domain/api/admin-key
```

这将返回当前有效的量子预测密钥，格式如下：
...
```json
{
  ...
  "currentKey": "abcd1234",
  "generatedAt": "2023-07-31T12:34:56.789Z"
}
```

> 注意：请妥善保管管理员密钥，不要泄露或在不安全的环境中使用。
